...
import { supabaseUrl, supabaseServiceKey, list } from './lib/supabase'
...
function LoginPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900 px-4">
      <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-8 w-full max-w-sm space-y-4">
        <h1 className="text-2xl font-bold text-center text-gray-800 dark:text-gray-100">Encuestas - Login</h1>
        <p className="text-sm text-center text-gray-500">Encuestas Horario Apertura</p>
        <input value={usuario} onChange={e => setUsuario(e.target.value)} placeholder="Usuario" className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-sm bg-white dark:bg-gray-700 dark:text-gray-100 outline-none focus:ring-2 focus:ring-green-500" />
        <input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="Contraseña" className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-sm bg-white dark:bg-gray-700 dark:text-gray-100 outline-none focus:ring-2 focus:ring-green-500" />
        {error && <p className="text-sm text-red-600">{error}</p>}
        <button type="submit" disabled={loading} className="w-full bg-green-600 hover:bg-green-700 text-white font-semibold py-2.5 rounded-lg transition cursor-pointer disabled:bg-gray-400">{loading ? 'Ingresando...' : 'Ingresar'}</button>
      </form>
    </div>
  )
}
...
function AppInner() {
  ...
  if (isAuthenticated === null) return <div className="flex items-center justify-center min-h-screen bg-gray-50 dark:bg-gray-900"><div className="animate-spin h-8 w-8 border-4 border-green-500 border-t-transparent rounded-full" /></div>
  if (!isAuthenticated) return <LoginPage />
  return (
    <div className="min-h-screen flex flex-col bg-gray-50 dark:bg-gray-900">
      <Header />
      <main className="flex-1 py-6"><EncuestaForm /></main>
    </div>
  )
}
